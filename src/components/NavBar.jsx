...
const NavBar = () => {
  ...
  return (
    <nav className='w-full flex py-6 justify-between items-center border-4 border-blue-800'>
      ...
      <div className='sm:hidden flex flex-1 justify-end items-center border-4 border-red-800'>
        ...
        <img 
          ...
          <div className={`${toggle ? 'flex' : 'hidden'} p-6 bg-black-gradient absolute top-20 right-0 mx-4 my-2 min-w-[140px] rounded-xl sidebar`}>
              ...
              {navLinks.map((nav, index) => (
                <li key={nav.id} className={`font-poppins font-normal cursor-pointer text-[16px]  ${index === navLinks.length - 1 ? 'mb-0' : 'mb-3'} text-white mr-10`} >
                  <a href={`#${nav.id}`} >
                    {nav.title}
                  </a>
                </li>
